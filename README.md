...
### Notion 連携（LINE 経由の分析結果を DB に保存）

- `NOTION_API_KEY` を設定すると、分析完了後に **指定データベースへ新規ページ**が追加されます（既定 DB ID: `7892876a-3b90-4614-b90f-8dc426e129a5`。`NOTION_DATABASE_ID` で上書き可）。
- **プロパティ**（コンサル分析 DB の列名と一致）:
  - `クライアント名`（タイトル）… LINE の表示名
  - `日付`（日付）… 実行日（今日）
  - `売上` / `スタッフ数`（リッチテキスト）… 「数字サマリー」から抽出した文言（列が `number` の場合のみ数値化を試みる）
  - `課題（緊急）` / `課題（中期）` / `課題（長期）`（リッチテキスト）… 🔴🟡🟢 ブロックから抽出
  - `アクション①今週` / `アクション②来月` / `アクション③3ヶ月後`（リッチテキスト）… ①②③ から抽出
  - `ステータス`（セレクト: 未対応・対応中・完了）… 固定で **未対応**
- インテグレーションを対象 DB に**接続**してください。
...
4. サービスの **Variables** に以下を登録する。
   ...
   - 任意: `NOTION_DATABASE_ID`（省略時は `7892876a-3b90-4614-b90f-8dc426e129a5`）
