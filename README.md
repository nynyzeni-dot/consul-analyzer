...
- **プロパティ**（DB 上の名前と一致させる）:
  - `クライアント名（タイトル）` … LINE の表示名（分析に使った名前）
  - `日付` … 実行日（今日）
  - `売上` / `スタッフ数` … 「数字サマリー」からルール抽出（`number` 型の列は数値に変換を試みる）
  - `課題（緊急）` / `課題（中期）` / `課題（長期）` … 🔴🟡🟢 ブロックから抽出
  - `アクション①今週` / `アクション②来月` / `アクション③3ヶ月後` … ①②③ 行から抽出
  - `ステータス` … 固定で **未対応**（`select` または `status` 型に対応）
- インテグレーションを対象 DB に**接続**し、ステータスに「未対応」オプションがあることを確認してください。
